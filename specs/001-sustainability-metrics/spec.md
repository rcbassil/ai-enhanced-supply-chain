...
- **FR-001**: System MUST load emission factors from `data/sustainability_config.json` before running any solver.
...
- **FR-006**: Emission factors MUST be configurable without code changes (via `sustainability_config.json`).
- **FR-007**: `query.py` MUST expose carbon metrics in tool responses for natural language queries.
- **FR-008**: System MUST NOT degrade existing solver performance by more than 10%.
